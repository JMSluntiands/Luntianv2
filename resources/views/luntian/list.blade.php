                ...
                <table class="luntian-table" id="luntianTable">
                    ...
                    <tbody>
                        @forelse($jobs ?? [] as $job)
                            ...
                            @endphp
                            <tr class="luntian-data-row lbs-data-row" data-job-id="{{ $job->job_id }}" data-job-units="{{ (int) ($job->units ?? 0) }}" data-update-url="{{ route('luntian.job.update', ['id' => $job->job_id], false) }}">
                                ...
                                <td class="luntian-td luntian-td-nowrap" data-label="Status" style="white-space: nowrap;">
                                    @include('partials.lbs-inline-status-cell', [
                                        ...
                                        'reference' => $referenceDisplay ?? ($job->job_reference_no ?? ''),
                                    ])
                                </td>
