...
@push('styles')
    ...
    <style>
        ...
        /* Mailbox table: compact layout, no stretching */
        #lbsMailboxTable.lbs-table-mailbox { min-width: 0; table-layout: auto; width: 100%; }
        .lbs-table-mailbox .lbs-col-action { width: 1%; white-space: nowrap; }
        .lbs-table-mailbox .lbs-col-log-date { width: 1%; white-space: nowrap; }
        .lbs-table-mailbox .lbs-col-reference { width: 1%; white-space: nowrap; }
        .lbs-table-mailbox .lbs-col-to { width: auto; min-width: 120px; max-width: 220px; }
        .lbs-table-mailbox .lbs-col-email-format { width: 1%; white-space: nowrap; }
        .lbs-table-mailbox .lbs-col-files { width: 1%; white-space: nowrap; }
        .lbs-table-mailbox .lbs-th,
        .lbs-table-mailbox .lbs-td { padding: 0.5rem 0.65rem; font-size: 0.8125rem; }
        .lbs-table-mailbox .lbs-th { padding-left: 0.75rem; padding-right: 0.75rem; }
        .lbs-mailbox-action-btns { display: flex; align-items: center; gap: 0.25rem; flex-wrap: nowrap; }
        .lbs-btn { display: inline-flex; align-items: center; justify-content: center; gap: 0.25rem; padding: 0.35rem 0.5rem; font-size: 0.75rem; font-weight: 600; border-radius: 6px; border: none; cursor: pointer; transition: background 0.2s, color 0.2s; }
        .lbs-btn.lbs-btn-icon { padding: 0.4rem; min-width: 32px; min-height: 32px; }
        .lbs-btn.lbs-btn-preview { padding: 0.35rem 0.6rem; }
        .lbs-btn svg { flex-shrink: 0; display: block; width: 16px; height: 16px; }
        .lbs-btn.lbs-btn-icon svg { width: 16px; height: 16px; }
        .lbs-btn-revert { background: rgba(239, 68, 68, 0.2); color: #fca5a5; }
        .lbs-btn-revert:hover { background: rgba(239, 68, 68, 0.35); color: #fecaca; }
        .lbs-btn-send { background: rgba(59, 130, 246, 0.25); color: #93c5fd; }
        .lbs-btn-send:hover { background: rgba(59, 130, 246, 0.4); color: #bfdbfe; }
        .lbs-btn-preview { background: rgba(59, 130, 246, 0.25); color: #93c5fd; }
        .lbs-btn-preview:hover { background: rgba(59, 130, 246, 0.4); color: #bfdbfe; }
        .lbs-link-uploaded-files { color: #60a5fa; text-decoration: none; font-size: 0.8125rem; white-space: nowrap; }
        .lbs-link-uploaded-files:hover { text-decoration: underline; }
        .lbs-no-files { color: #64748b; }
        html[data-theme="light"] .lbs-btn-revert { background: rgba(239, 68, 68, 0.15); color: #b91c1c; }
        html[data-theme="light"] .lbs-btn-revert:hover { background: rgba(239, 68, 68, 0.25); }
        html[data-theme="light"] .lbs-btn-send,
        html[data-theme="light"] .lbs-btn-preview { background: rgba(59, 130, 246, 0.15); color: #1d4ed8; }
        html[data-theme="light"] .lbs-btn-send:hover,
        html[data-theme="light"] .lbs-btn-preview:hover { background: rgba(59, 130, 246, 0.25); }
        html[data-theme="light"] .lbs-link-uploaded-files { color: #2563eb; }
        .revert-modal-confirm p { margin: 0; }
        .revert-modal-countdown { text-align: center; padding: 0.5rem 0; }
        .revert-countdown-text { font-size: 0.9375rem; color: #94a3b8; margin: 0 0 1rem 0; }
        .revert-countdown-number { font-size: 4rem; font-weight: 800; color: #f97316; line-height: 1; letter-spacing: -0.05em; min-height: 4rem; display: flex; align-items: center; justify-content: center; animation: revert-countdown-pop 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
        .revert-countdown-cancel-hint { font-size: 0.8125rem; color: #64748b; margin: 1rem 0 0 0; }
        @keyframes revert-countdown-pop { 0% { opacity: 0; transform: scale(0.3); } 70% { transform: scale(1.1); } 100% { opacity: 1; transform: scale(1); } }
        html[data-theme="light"] .revert-countdown-text { color: #64748b; }
        html[data-theme="light"] .revert-countdown-number { color: #ea580c; }
        .email-preview-modal-box { max-width: 520px; }
        .email-preview-modal-header { display: flex; align-items: center; justify-content: space-between; padding: 1rem 1.25rem; border-bottom: 1px solid #334155; }
        .email-preview-modal-header .modal-title { margin: 0; font-size: 1.125rem; font-weight: 700; color: #e2e8f0; }
        .email-preview-close { background: none; border: none; font-size: 1.5rem; line-height: 1; color: #94a3b8; cursor: pointer; padding: 0.25rem; margin: -0.25rem -0.25rem -0.25rem 0; }
        .email-preview-close:hover { color: #e2e8f0; }
        .email-preview-body { padding: 1.5rem 1.25rem; max-height: 70vh; overflow-y: auto; }
        .email-preview-content { text-align: center; }
        .email-preview-logo-wrap { margin-bottom: 0.5rem; }
        .email-preview-logo { max-width: 180px; height: auto; display: block; margin: 0 auto; }
        .email-preview-logo-fallback { font-size: 1.75rem; font-weight: 800; color: #f97316; }
        .email-preview-tagline { font-size: 0.8125rem; color: #64748b; margin: 0 0 0.25rem 0; }
        .email-preview-services { font-size: 0.75rem; color: #f97316; margin: 0 0 1.25rem 0; letter-spacing: 0.02em; }
        .email-preview-greeting { font-size: 1.25rem; font-weight: 700; color: #e2e8f0; margin: 0 0 0.5rem 0; }
        .email-preview-ref-wrap { margin: 0 0 0.35rem 0; }
        .email-preview-ref { font-size: 1.75rem; font-weight: 800; color: #f97316; display: inline-block; }
        .email-preview-status-text { font-size: 0.9375rem; color: #94a3b8; margin: 0 0 0.2rem 0; }
        .email-preview-status-value { font-size: 1.125rem; font-weight: 700; color: #f97316; margin: 0 0 1rem 0; }
        .email-preview-assessor, .email-preview-assessor-email { font-size: 0.875rem; color: #e2e8f0; margin: 0 0 0.25rem 0; text-align: left; }
        .email-preview-assessor-email { margin-bottom: 1rem; }
        .email-preview-link { color: #60a5fa; text-decoration: none; }
        .email-preview-link:hover { text-decoration: underline; }
        .email-preview-notes-title { font-size: 0.875rem; font-weight: 700; color: #e2e8f0; margin: 0 0 0.5rem 0; text-align: left; }
        .email-preview-notes { font-size: 0.875rem; color: #94a3b8; line-height: 1.5; text-align: left; }
        .email-preview-notes a { color: #60a5fa; }
        html[data-theme="light"] .email-preview-modal-header { border-bottom-color: #e2e8f0; }
        html[data-theme="light"] .email-preview-modal-header .modal-title { color: #1e293b; }
        html[data-theme="light"] .email-preview-greeting { color: #1e293b; }
        html[data-theme="light"] .email-preview-ref { color: #ea580c; }
        html[data-theme="light"] .email-preview-status-value { color: #ea580c; }
        html[data-theme="light"] .email-preview-assessor, html[data-theme="light"] .email-preview-assessor-email { color: #334155; }
        html[data-theme="light"] .email-preview-notes-title { color: #1e293b; }
        html[data-theme="light"] .email-preview-notes { color: #64748b; }
        .send-email-modal-box { max-width: 380px; }
        .send-email-modal-body { padding: 1.5rem 1.25rem; }
        .send-email-steps { display: flex; flex-direction: column; gap: 1rem; }
        .send-email-step { display: flex; align-items: center; gap: 0.75rem; font-size: 0.9375rem; color: #e2e8f0; opacity: 0.7; transition: opacity 0.3s ease; }
        .send-email-step.active { opacity: 1; font-weight: 500; }
        .send-email-step.done { opacity: 1; }
        .send-email-step.done .send-email-step-icon { background: rgba(34, 197, 94, 0.3); color: #86efac; }
        .send-email-step-icon { display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 50%; flex-shrink: 0; font-size: 0.875rem; font-weight: 700; }
        .send-email-step-spinner { background: rgba(59, 130, 246, 0.25); border: 2px solid rgba(59, 130, 246, 0.5); border-top-color: #60a5fa; animation: send-email-spin 0.7s linear infinite; }
        .send-email-step-pending { background: #334155; color: #94a3b8; }
        .send-email-step-check { background: rgba(34, 197, 94, 0.3); color: #86efac; }
        .send-email-done { margin-top: 0.5rem; padding-top: 0.75rem; border-top: 1px solid #334155; }
        .send-email-done .send-email-step-text { font-weight: 700; color: #86efac; }
        @keyframes send-email-spin { to { transform: rotate(360deg); } }
        .send-email-modal-box .modal-enter { animation: send-modal-in 0.35s ease-out; }
        @keyframes send-modal-in { from { opacity: 0; transform: scale(0.95); } to { opacity: 1; transform: scale(1); } }
        html[data-theme="light"] .send-email-step { color: #334155; }
        html[data-theme="light"] .send-email-step.done .send-email-step-icon { background: rgba(34, 197, 94, 0.2); color: #16a34a; }
        html[data-theme="light"] .send-email-step-pending { background: #e2e8f0; color: #64748b; }
        html[data-theme="light"] .send-email-done .send-email-step-text { color: #16a34a; }
    </style>
@endpush
