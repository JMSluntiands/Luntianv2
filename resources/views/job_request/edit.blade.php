    ...
    <div class="w-full">
        ...
        <div class="mb-6 flex flex-wrap items-start gap-4">
            ...
            <div class="min-w-0 flex-1">
                <h1 class="mb-1.5 text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Edit Job Request</h1>
                <p class="text-slate-600 dark:text-slate-400">Update job request #{{ $jobRequest->id }} used when logging LBS jobs.</p>
            </div>
        </div>
        ...
        <form action="{{ route('job_request.update', $jobRequest) }}" method="POST" class="space-y-6" autocomplete="off">
            @csrf
            @method('PUT')
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                id="job_request_id"
                                name="job_request_id"
                                value="{{ old('job_request_id', $jobRequest->job_request_id) }}"
                                maxlength="50"
                                required
                                autocomplete="off"
                        ...
                        <div>
                            ...
                            <input
                                ...
                                value="{{ old('job_request_type', $jobRequest->job_request_type) }}"
                                maxlength="255"
                                required
                                autocomplete="off"
                        ...
                        <div class="flex w-full flex-col justify-between rounded-xl bg-slate-50 px-4 py-4 text-sm text-slate-700 ring-1 ring-slate-200 dark:bg-slate-800/60 dark:text-slate-200 dark:ring-slate-700">
                            ...
                            <div class="space-y-2">
                                ...
                                <p id="jrPreviewClient" class="rounded-lg bg-white px-3 py-2 text-sm font-medium text-slate-800 ring-1 ring-slate-200 dark:bg-slate-900/40 dark:text-slate-100 dark:ring-slate-700">
                                    {{ old('client_code', $jobRequest->client_code ?? 'SHG') }}
                                </p>
                            </div>
                            <div class="mt-3 space-y-2">
                                ...
                                <p id="jrPreviewId" class="truncate text-sm font-semibold text-slate-800 dark:text-slate-100">
                                    {{ old('job_request_id', $jobRequest->job_request_id ?? 'EA_LBS_1SNatHERS') }}
                                </p>
                                <p id="jrPreviewType" class="text-xs text-slate-500 dark:text-slate-400">
                                    {{ old('job_request_type', $jobRequest->job_request_type ?? '1S NatHERS Base Model') }}
                                </p>
                            </div>
...
@push('scripts')
    ...
    <script>
        (function() {
            var form = document.querySelector('form[action="{{ route('job_request.update', $jobRequest) }}"]');
