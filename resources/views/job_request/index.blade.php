    ...
    <div class="w-full">
        ...
        <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
            <div class="flex items-start gap-4">
                ...
                <div class="min-w-0">
                    <h1 class="mb-1.5 text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Job Request</h1>
                    <p class="text-slate-600 dark:text-slate-400">View and manage job request types (client code, request ID, and type) used when creating LBS jobs.</p>
                </div>
            </div>
            <a href="{{ route('job_request.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md transition-all hover:bg-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900">
        ...
        <form method="GET" action="{{ route('job_request.index') }}" class="mb-4 flex flex-wrap items-end gap-3" id="jobRequestFilterForm" autocomplete="off">
            ...
            <div class="min-w-60 flex-1">
                ...
                <input type="search" id="jobRequestSearch" name="q" value="{{ $search }}" placeholder="Request ID or type..." class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 placeholder-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/25 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 dark:placeholder-slate-500">
            </div>
                ...
                <table class="w-full min-w-[820px] border-collapse text-sm" id="jobRequestTable">
                    ...
                    <thead>
                        <tr class="border-b border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800/80">
                            ...
                            <th class="px-5 py-3.5 text-left font-semibold text-slate-600 dark:text-slate-300">Job Request ID</th>
                            <th class="px-5 py-3.5 text-left font-semibold text-slate-600 dark:text-slate-300">Job Request Type</th>
                    ...
                    <tbody>
                        @forelse($jobRequests as $jr)
                            <tr class="border-b border-slate-100 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800/50">
                                <td class="px-5 py-3.5 text-slate-600 dark:text-slate-400">{{ $jr->id }}</td>
                                ...
                                <td class="px-5 py-3.5">
                                    <span class="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:ring-emerald-600/60">
                                ...
                                <td class="px-5 py-3.5 text-right">
                                    <div class="flex items-center justify-end gap-1">
                                        <a href="{{ route('job_request.edit', $jr) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg text-slate-500 transition-colors hover:bg-emerald-500/15 hover:text-emerald-600 dark:text-slate-400 dark:hover:text-emerald-400" title="Edit" aria-label="Edit">
                                        ...
                                        <form action="{{ route('job_request.destroy', $jr) }}" method="POST" class="inline" data-delete-form autocomplete="off">
                                            @csrf
                                            @method('DELETE')
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 opacity-0 pointer-events-none transition-opacity duration-200 backdrop-blur-sm" id="deleteJobRequestModal" role="dialog" aria-labelledby="deleteJobRequestModalTitle" aria-modal="true">
        <div class="w-full max-w-sm overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-600 dark:bg-slate-800" role="document">
            <div class="flex items-center gap-3 px-5 py-5">
                ...
                <h2 class="text-lg font-bold text-slate-800 dark:text-white" id="deleteJobRequestModalTitle">Delete Job Request</h2>
            </div>
            <div class="px-5 pb-4">
                <div id="deleteModalConfirm">
                    <p class="text-slate-600 dark:text-slate-300">Are you sure you want to delete this job request? This action cannot be undone.</p>
                </div>
            ...
            <div class="flex justify-end gap-3 border-t border-slate-200 px-5 py-4 dark:border-slate-700">
                <button type="button" class="rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600 dark:focus:ring-offset-slate-800" id="deleteJobRequestModalCancel">Cancel</button>
                <button type="button" class="rounded-lg bg-red-600 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-slate-800" id="deleteJobRequestModalConfirm"><span class="btn-text">Delete</span></button>
            </div>
        </div>
    </div>
@endsection
