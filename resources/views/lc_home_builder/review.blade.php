...
@section('content')
    @php
        $__assign = \App\Models\User::assignmentInitialsViewData('lc_home_builder');
                ...
                <table class="lbs-table w-full min-w-[1320px] table-fixed border-collapse text-sm" id="lbsTable">
                    ...
                    <tbody>
                        ...
                        @foreach($rows as $row)
                            ...
                            @endphp
                            <tr class="lbs-data-row border-b border-slate-200 align-middle text-slate-800 dark:border-slate-700 dark:text-slate-200" data-job-units="{{ (int) ($row->units ?? 0) }}" data-update-url="{{ route('lc_home_builder.update', ['id' => $row->id], false) }}">
