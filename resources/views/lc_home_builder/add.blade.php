...
@section('content')
    <div class="w-full max-w-full px-0">
        {{-- Page Header --}}
        <div class="mb-8">
            <h1 class="mb-2 text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Add New Job (LC HOME BUILDER)</h1>
            <p class="text-slate-500 dark:text-slate-400">Fill in the form below to create a new LC HOME BUILDER job.</p>
        </div>

        <form id="lcHomeBuilderAddForm" action="#" method="POST" autocomplete="off" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/50 overflow-hidden">
                <div class="flex items-center justify-between gap-4 border-b border-slate-200 bg-slate-50/80 px-5 py-4 dark:border-slate-700 dark:bg-slate-800/80">
                    ...
                    <span id="lcHomeBuilderJobReferenceContent" class="rounded-lg bg-slate-200/80 px-3 py-1.5 font-mono text-sm font-medium text-slate-700 dark:bg-slate-700 dark:text-slate-300">JOB0903-001</span>
                </div>
                <div class="p-5">
                    <div class="space-y-5">
                        @php
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <span id="job_number_error_icon" class="pointer-events-none absolute right-3 top-1/2 hidden -translate-y-1/2 rounded-full bg-red-100 p-1 text-red-600 dark:bg-red-900/40 dark:text-red-400" aria-hidden="true">
                                    <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
                                </span>
                            </div>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/50 overflow-hidden">
                ...
                <div class="p-5">
                    <div>
                        <label for="lc-notes-body" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Notes (LC HOME BUILDER)</label>
                        <input type="hidden" name="notes" id="lc_notes" autocomplete="off">
                        <div class="overflow-hidden rounded-lg border border-slate-300 dark:border-slate-600">
                            <div class="flex items-center gap-1 border-b border-slate-200 bg-slate-50 px-2 py-1.5 dark:border-slate-600 dark:bg-slate-800/80">
                                <button type="button" class="lc-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="bold" title="Bold"><span class="font-bold">B</span></button>
                                <button type="button" class="lc-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="italic" title="Italic"><span class="italic">I</span></button>
                                <button type="button" class="lc-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="underline" title="Underline"><span class="underline">U</span></button>
                                <span class="mx-1 h-5 w-px bg-slate-300 dark:bg-slate-600"></span>
                                <button type="button" class="lc-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="insertOrderedList" title="Numbered list">
                                ...
                                <button type="button" class="lc-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="insertUnorderedList" title="Bullets">
                            ...
                            <div id="lc-notes-body" contenteditable="true" data-placeholder="Write notes here"
                ...
                <div class="p-5">
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            ...
                            <label for="lc_upload_plans" class="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50/50 py-4 px-3 text-center transition-colors hover:border-emerald-400 hover:bg-emerald-50/50 dark:border-slate-600 dark:bg-slate-800/50 dark:hover:border-emerald-600 dark:hover:bg-emerald-950/30">
                                ...
                                <input type="file" id="lc_upload_plans" name="upload_plans[]" class="hidden" multiple tabindex="-1">
                            </label>
                            <div id="lc-plans-file-list" class="mt-2 min-h-0 space-y-1 text-xs text-slate-600 dark:text-slate-400"></div>
                            <button type="button" id="lc-plans-clear" class="mt-1 hidden text-xs text-slate-500 hover:text-red-600 dark:text-slate-400 dark:hover:text-red-400">Clear</button>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Upload Document</label>
                            <label for="lc_upload_document" class="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50/50 py-4 px-3 text-center transition-colors hover:border-emerald-400 hover:bg-emerald-50/50 dark:border-slate-600 dark:bg-slate-800/50 dark:hover:border-emerald-600 dark:hover:bg-emerald-950/30">
                                ...
                                <span class="mt-0.5 text-xs text-slate-500 dark:text-slate-500">or</span>
                                <span class="rounded bg-emerald-500/20 px-2 py-0.5 text-xs font-medium text-emerald-600 dark:text-emerald-400">browse</span>
                                <input type="file" id="lc_upload_document" name="upload_document[]" class="hidden" multiple tabindex="-1">
                            </label>
                            <div id="lc-docs-file-list" class="mt-2 min-h-0 space-y-1 text-xs text-slate-600 dark:text-slate-400"></div>
                            <button type="button" id="lc-docs-clear" class="mt-1 hidden text-xs text-slate-500 hover:text-red-600 dark:text-slate-400 dark:hover:text-red-400">Clear</button>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            ...
                            <select id="assigned_to" name="assigned_to" class="select2-single assignment-user-select w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-slate-800 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/25 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                                @include('partials.assignment-user-options', ['selected' => '', 'includeSelectPlaceholder' => true, 'includeGm' => false])
                            </select>
                        </div>
                        <div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                <button type="button" id="submitLCBtn"
                ...
                <a href="{{ route('lc_home_builder.list') }}" class="rounded-xl border border-slate-300 px-5 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800">Cancel</a>
            </div>
        </form>
    </div>
@endsection
...
@push('scripts')
    ...
    <script>
        $(function() {
            var notesBody = document.getElementById('lc-notes-body');
            var noteBtns = document.querySelectorAll('.lc-notes-btn');
            ...
            function initLcSelect2() {
                $('#lcHomeBuilderAddForm select:not(.assignment-user-select)').each(function() {
                    var $el = $(this);
            ...
            renderFileList('lc_upload_plans', 'lc-plans-file-list', 'lc-plans-clear');
            renderFileList('lc_upload_document', 'lc-docs-file-list', 'lc-docs-clear');
            ...
            $btn.on('click', function(e) {
                e.preventDefault();
                document.getElementById('lc_notes').value = notesBody.innerHTML;
                ...
                var formEl = document.getElementById('lcHomeBuilderAddForm');
                var formData = new FormData(formEl);
                var headerRef = ($('#lcHomeBuilderJobReferenceContent').text() || '').trim();
                if (headerRef) {
                    formData.append('header_reference', headerRef);
                }

                $.ajax({
                    url: '{{ route("lc_home_builder.store") }}',
                    method: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function(resp) {
                        if (resp && resp.status === 'success') {
                            ...
                            formEl.reset();
                            notesBody.innerHTML = '';
                            document.getElementById('lc_upload_plans') && document.getElementById('lc_upload_plans').dispatchEvent(new Event('change'));
                            document.getElementById('lc_upload_document') && document.getElementById('lc_upload_document').dispatchEvent(new Event('change'));
                            initLcSelect2();
                            showLcAfterSavePrompt(resp.job_id);
                        } else {
            ...
            function showLcAfterSavePrompt(jobId) {
                var sendSlackUrl = '{{ url("dashboard/lc-home-builder/job") }}/' + jobId + '/send-slack';
                var listUrl = '{{ route("lc_home_builder.list") }}';
                    ...
                    '<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 lbs-after-save-overlay">' +
                        '<div class="w-full max-w-sm rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800 overflow-hidden lbs-after-save-dialog">' +
                            '<div class="p-6 text-center lbs-modal-step lbs-modal-step-question">' +
                                ...
                                '<p class="mt-4 text-sm text-slate-500 dark:text-slate-400">Do you want to create another LC HOME BUILDER job?</p>' +
                                '<div class="mt-6 flex gap-3">' +
                                    '<button type="button" data-lc-go-list class="cursor-pointer flex-1 rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700">Go to LC HOME BUILDER list</button>' +
                                    '<button type="button" data-lc-new-job class="cursor-pointer flex-1 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-emerald-500">Create another job</button>' +
                                '</div>' +
                            '</div>' +
                ...
                function proceedAfterSlack(action) {
                    ...
                    var $goListBtn = $overlay.find('[data-lc-go-list]');
                    var $newJobBtn = $overlay.find('[data-lc-new-job]');
                ...
                $overlay.find('[data-lc-new-job]').on('click', function() {
                    proceedAfterSlack('stay');
                });

                $overlay.find('[data-lc-go-list]').on('click', function() {
