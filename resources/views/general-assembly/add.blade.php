...
@section('content')
    <div class="w-full max-w-full px-0">
        ...
        <div class="mb-8">
            ...
            <p class="text-slate-500 dark:text-slate-400">Fill in the form below to create a new Generic Assessment job.</p>
        </div>
    ...
    <script>
        $(function() {
            ...
            $btn.on('click', function(e) {
                ...
                $.ajax({
                    url: '{{ $storeRoute ?? route('general_assembly.store') }}',
                    method: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
            ...
            function showLbsAfterSavePrompt(jobId, submissionEmailEnabled) {
                ...
                var sendSlackUrl = '{{ $sendSlackBaseUrl ?? url('dashboard/general-assembly/job') }}' + '/' + jobId + '/send-slack';
                var sendUrl = '{{ $sendSubmissionBaseUrl ?? url('dashboard/general-assembly/job') }}' + '/' + jobId + '/send-submission-email';
                var listUrl = '{{ $listUrl ?? route('general_assembly.list') }}';
                    ...
                    '<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 lbs-after-save-overlay">' +
                        '<div class="w-full max-w-sm rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800 overflow-hidden lbs-after-save-dialog">' +
                            '<div class="p-6 text-center lbs-modal-step lbs-modal-step-question">' +
                                ...
                                '<p class="mt-4 text-sm text-slate-500 dark:text-slate-400">Do you want to create another Generic Assessment job?</p>' +
                                '<div class="mt-6 flex gap-3">' +
                                    '<button type="button" data-lbs-go-list class="cursor-pointer flex-1 rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700">Go to Generic Assessment List</button>' +
