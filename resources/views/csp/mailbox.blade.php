...
@section('title', 'CSP Mailbox')

@section('body_class', 'page-lbs-mailbox page-csp-mailbox')

@section('content')
    @php
        $branchLabel = 'CSP';
        $updateRoute = 'csp.update';
        $jobBaseUrl = url('/dashboard/csp/job');
    @endphp
    <div class="block max-w-full pb-0">
        <div class="mb-7 flex flex-wrap items-start justify-between gap-4">
            ...
            <div class="shrink-0">
                <label for="cspMailboxSearch" class="mb-1.5 block text-xs font-semibold text-slate-600 dark:text-slate-400">Search</label>
                <div class="relative flex min-w-[260px] items-center">
                    ...
                    <input type="search" id="cspMailboxSearch" class="w-full rounded-lg border border-slate-300 bg-slate-50 py-2 pl-9 pr-3.5 text-sm text-slate-800 placeholder-slate-400 transition-colors focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-600/25 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:placeholder-slate-500 dark:focus:border-blue-700 dark:focus:ring-blue-700/25" placeholder="Search by reference, recipient..." autocomplete="off" aria-label="Search {{ $branchLabel }} mailbox">
                </div>
            </div>
        </div>
        ...
        <div class="max-w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow dark:border-slate-700 dark:bg-slate-900">
            <div class="max-w-full overflow-x-auto">
                <table class="w-full min-w-[800px] table-fixed border-collapse text-sm" id="cspMailboxTable">
                    <colgroup>
                    ...
                    <tbody>
                        @forelse($jobs as $index => $job)
                            ...
                            @endphp
                            <tr class="lbs-data-row csp-mailbox-row border-b border-slate-200 align-middle text-slate-800 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-white/5" data-job-id="{{ $job->job_id }}" data-update-url="{{ route($updateRoute, ['id' => $job->job_id], false) }}">
                                ...
                                <td class="border-b border-slate-200 px-4 py-3 align-middle text-slate-800 dark:border-slate-700 dark:text-slate-200" data-label="Print (PDF)">
                                    <a href="{{ route('csp.job.printCompliance', ['id' => $job->job_id]) }}" target="_blank" rel="noopener noreferrer" class="text-blue-600 underline hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">Open compliance PDF</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
...
@push('scripts')
    ...
    <script>
        (function() {
            var table = document.getElementById('cspMailboxTable');
            var searchInput = document.getElementById('cspMailboxSearch');
            ...
            if (table && searchInput) {
                searchInput.addEventListener('input', function() {
                    ...
                    var rows = table.querySelectorAll('tbody tr.csp-mailbox-row');
