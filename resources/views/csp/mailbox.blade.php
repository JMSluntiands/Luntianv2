                ...
                <table class="w-full min-w-[800px] table-fixed border-collapse text-sm" id="cspMailboxTable">
                    ...
                    <tbody>
                        @forelse($jobs as $index => $job)
                            ...
                            @endphp
                            <tr class="lbs-data-row csp-mailbox-row border-b border-slate-200 align-middle text-slate-800 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-white/5" data-job-id="{{ $job->job_id }}" data-update-url="{{ route($updateRoute, ['id' => $job->job_id]) }}">
