        ...
        <form id="bluinqAddForm" action="#" method="POST" autocomplete="off" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/50 overflow-hidden">
                <div class="flex items-center justify-between gap-4 border-b border-slate-200 bg-slate-50/80 px-5 py-4 dark:border-slate-700 dark:bg-slate-800/80">
                    ...
                    <span id="bluinqJobReferenceContent" class="rounded-lg bg-slate-200/80 px-3 py-1.5 font-mono text-sm font-medium text-slate-700 dark:bg-slate-700 dark:text-slate-300">JOB0903-001</span>
                </div>
                <div class="p-5">
                    <div class="space-y-5">
                        @php
                    ...
                    <div>
                        ...
                        <div class="overflow-hidden rounded-lg border border-slate-300 dark:border-slate-600">
                            <div class="flex items-center gap-1 border-b border-slate-200 bg-slate-50 px-2 py-1.5 dark:border-slate-600 dark:bg-slate-800/80">
                                <button type="button" class="bluinq-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="bold" title="Bold"><span class="font-bold">B</span></button>
                                <button type="button" class="bluinq-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="italic" title="Italic"><span class="italic">I</span></button>
                                <button type="button" class="bluinq-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="underline" title="Underline"><span class="underline">U</span></button>
                                <span class="mx-1 h-5 w-px bg-slate-300 dark:bg-slate-600"></span>
                                <button type="button" class="bluinq-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="insertOrderedList" title="Numbered list">
                                ...
                                <button type="button" class="bluinq-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="insertUnorderedList" title="Bullets">
                            ...
                            <div id="bluinq-notes-body" contenteditable="true" data-placeholder="Write notes here"
                ...
                <div class="p-5">
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            ...
                            <label for="bluinq_upload_plans" class="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50/50 py-4 px-3 text-center transition-colors hover:border-emerald-400 hover:bg-emerald-50/50 dark:border-slate-600 dark:bg-slate-800/50 dark:hover:border-emerald-600 dark:hover:bg-emerald-950/30">
                                ...
                                <input type="file" id="bluinq_upload_plans" name="upload_plans[]" class="hidden" multiple tabindex="-1">
                            </label>
                            <div id="bluinq-plans-file-list" class="mt-2 min-h-0 space-y-1 text-xs text-slate-600 dark:text-slate-400"></div>
                            <button type="button" id="bluinq-plans-clear" class="mt-1 hidden text-xs text-slate-500 hover:text-red-600 dark:text-slate-400 dark:hover:text-red-400">Clear</button>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Upload Document</label>
                            <label for="bluinq_upload_document" class="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50/50 py-4 px-3 text-center transition-colors hover:border-emerald-400 hover:bg-emerald-50/50 dark:border-slate-600 dark:bg-slate-800/50 dark:hover:border-emerald-600 dark:hover:bg-emerald-950/30">
                                ...
                                <span class="mt-0.5 text-xs text-slate-500 dark:text-slate-500">or</span>
                                <span class="rounded bg-emerald-500/20 px-2 py-0.5 text-xs font-medium text-emerald-600 dark:text-emerald-400">browse</span>
                                <input type="file" id="bluinq_upload_document" name="upload_document[]" class="hidden" multiple tabindex="-1">
                            </label>
                            <div id="bluinq-docs-file-list" class="mt-2 min-h-0 space-y-1 text-xs text-slate-600 dark:text-slate-400"></div>
                            <button type="button" id="bluinq-docs-clear" class="mt-1 hidden text-xs text-slate-500 hover:text-red-600 dark:text-slate-400 dark:hover:text-red-400">Clear</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('bluinq.list') }}" class="rounded-xl border border-slate-300 px-5 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800">Cancel</a>
            </div>
        </form>
    </div>
@endsection
    ...
    <script>
        $(function() {
            ...
            function initBluinqSelect2() {
                $('#bluinqAddForm select:not(.assignment-user-select)').each(function() {
                    var $el = $(this);
            ...
            initBluinqSelect2();
            initAssignmentUserSelect2($('#bluinqAddForm'));
            ...
            renderFileList('bluinq_upload_plans', 'bluinq-plans-file-list', 'bluinq-plans-clear');
            renderFileList('bluinq_upload_document', 'bluinq-docs-file-list', 'bluinq-docs-clear');
            ...
            $btn.on('click', function(e) {
                ...
                var formEl = document.getElementById('bluinqAddForm');
                var formData = new FormData(formEl);
                var headerRef = ($('#bluinqJobReferenceContent').text() || '').trim();
                if (headerRef) {
                    formData.append('header_reference', headerRef);
                }

                $.ajax({
                    url: '{{ route("bluinq.store") }}',
                    method: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function(resp) {
                        if (resp && resp.status === 'success') {
                            ...
                            formEl.reset();
                            notesBody.innerHTML = '';
                            document.getElementById('bluinq_upload_plans') && document.getElementById('bluinq_upload_plans').dispatchEvent(new Event('change'));
                            document.getElementById('bluinq_upload_document') && document.getElementById('bluinq_upload_document').dispatchEvent(new Event('change'));
                            initBluinqSelect2();
            ...
            function showBluinqAfterSavePrompt(jobId) {
                var sendSlackUrl = '{{ url("dashboard/bluinq/job") }}/' + jobId + '/send-slack';
                var listUrl = '{{ route("bluinq.list") }}';
                ...
                function proceedAfterSlack(action) {
                    ...
                    var $goListBtn = $overlay.find('[data-bluinq-go-list]');
                    var $newJobBtn = $overlay.find('[data-bluinq-new-job]');
                ...
                $overlay.find('[data-bluinq-new-job]').on('click', function() {
                    proceedAfterSlack('stay');
                });

                $overlay.find('[data-bluinq-go-list]').on('click', function() {
