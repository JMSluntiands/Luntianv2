...
@section('content')
    ...
    <div class="block max-w-full pb-0">
        <div class="mb-7 flex flex-wrap items-start justify-between gap-4">
            ...
            <div class="shrink-0">
                <label for="bluinqMailboxSearch" class="mb-1.5 block text-xs font-semibold text-slate-600 dark:text-slate-400">Search</label>
                <div class="relative flex min-w-[260px] items-center">
                    ...
                    <input type="search" id="bluinqMailboxSearch" class="w-full rounded-lg border border-slate-300 bg-slate-50 py-2 pl-9 pr-3.5 text-sm text-slate-800 placeholder-slate-400 transition-colors focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-600/25 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:placeholder-slate-500 dark:focus:border-blue-700 dark:focus:ring-blue-700/25" placeholder="Search by reference, recipient..." autocomplete="off" aria-label="Search {{ $branchLabel }} mailbox">
                </div>
            </div>
        </div>
        ...
        <div class="max-w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow dark:border-slate-700 dark:bg-slate-900">
            <div class="max-w-full overflow-x-auto">
                <table class="w-full min-w-[800px] table-fixed border-collapse text-sm" id="bluinqMailboxTable">
                    <colgroup>
                    ...
                    <tbody>
                        @forelse($jobs as $index => $job)
                            ...
                            @endphp
                            <tr class="lbs-data-row bluinq-mailbox-row border-b border-slate-200 align-middle text-slate-800 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-white/5" data-job-id="{{ $job->job_id }}" data-update-url="{{ route($updateRoute, ['id' => $job->job_id], false) }}">
...
@push('scripts')
    ...
    <script>
        (function() {
            var table = document.getElementById('bluinqMailboxTable');
            var searchInput = document.getElementById('bluinqMailboxSearch');
            ...
            if (table && searchInput) {
                searchInput.addEventListener('input', function() {
                    ...
                    var rows = table.querySelectorAll('tbody tr.bluinq-mailbox-row');
            ...
            document.querySelectorAll('[data-revert-job]').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    var row = this.closest('tr.bluinq-mailbox-row');
