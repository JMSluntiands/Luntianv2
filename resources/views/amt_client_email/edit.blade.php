...
@section('title', 'Edit A&M Client Email')

@section('body_class', 'page-amt-client-email-edit')
    ...
    <div class="w-full">
        <div class="mb-6 flex flex-wrap items-start gap-4">
            ...
            <div class="min-w-0 flex-1">
                <h1 class="mb-1.5 text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Edit A&amp;M client email</h1>
                <p class="text-slate-600 dark:text-slate-400">Update record #{{ $clientEmailAmt->id }}.</p>
            </div>
        </div>
        ...
        <form action="{{ route('amt_client_email.update', $clientEmailAmt) }}" method="POST" class="space-y-6" autocomplete="off">
            @csrf
            @method('PUT')
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800/60">
                <div>
                    ...
                    <input type="email" id="email" name="email" value="{{ old('email', $clientEmailAmt->email) }}" required
