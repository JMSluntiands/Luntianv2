...
@section('title', 'Add A&M Client Email')

@section('body_class', 'page-amt-client-email-create')
    ...
    <div class="w-full">
        <div class="mb-6 flex flex-wrap items-start gap-4">
            ...
            <div class="min-w-0 flex-1">
                <h1 class="mb-1.5 text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Add A&amp;M client email</h1>
                <p class="text-slate-600 dark:text-slate-400">Save a recipient address for A&amp;M notifications.</p>
            </div>
        </div>
        ...
        <form action="{{ route('amt_client_email.store') }}" method="POST" class="space-y-6" autocomplete="off">
            @csrf
            ...
            <div class="flex flex-wrap items-center gap-3">
                <a href="{{ route('amt_client_email.index') }}" class="inline-flex items-center rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600">Cancel</a>
                <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md transition-all hover:bg-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900">Save</button>
            </div>
        </form>
    </div>
@endsection
