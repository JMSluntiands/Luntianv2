...
@section('body_class', 'page-staff-create')

@section('content')
    <div class="staff-form-page">
        <div class="staff-form-header">
            <h1 class="staff-form-title">Add Staff</h1>
            <p class="staff-form-subtitle">Create a new staff account.</p>
        </div>
        ...
        <form action="{{ route('staff.store') }}" method="POST" class="staff-form" autocomplete="off">
            @csrf
            <div class="staff-card">
                <div class="staff-form-grid">
                    <div class="staff-form-group">
                        <label class="staff-label" for="staff_id">Code</label>
                        <input type="text" id="staff_id" name="staff_id" class="staff-input" placeholder="e.g. SB, JS" value="{{ old('staff_id') }}" autocomplete="off">
                    </div>
                    <div class="staff-form-group">
                        <label class="staff-label" for="name">Name</label>
                        <input type="text" id="name" name="name" class="staff-input" placeholder="Full name" value="{{ old('name') }}" autocomplete="off">
                    </div>
                </div>
            </div>
...
@push('styles')
    <style>
        ...
        body.page-staff-create .content { padding-bottom: 0; }
