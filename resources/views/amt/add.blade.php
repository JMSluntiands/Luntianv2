...
@section('title', 'Add New Job (A&M)')

@section('body_class', 'page-amt-add')

@section('content')
    <div class="w-full max-w-full px-0">
        <div class="mb-8">
            <h1 class="mb-2 text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">Add New Job (A&amp;M)</h1>
            <p class="text-slate-500 dark:text-slate-400">Fill in the form below to create a new A&amp;M job.</p>
        </div>

        <form id="amtAddForm" action="#" method="POST" autocomplete="off" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/50 overflow-hidden">
                <div class="flex items-center justify-between gap-4 border-b border-slate-200 bg-slate-50/80 px-5 py-4 dark:border-slate-700 dark:bg-slate-800/80">
                    ...
                    <span id="amtJobReferenceContent" class="rounded-lg bg-slate-200/80 px-3 py-1.5 font-mono text-sm font-medium text-slate-700 dark:bg-slate-700 dark:text-slate-300">JOB0903-001</span>
                </div>
                <div class="p-5">
                    <div class="space-y-5">
                        @php
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/50 overflow-hidden">
                ...
                <div class="p-5">
                    <div>
                        <label for="amt-notes-body" class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Notes (A&amp;M)</label>
                        <input type="hidden" name="notes" id="amt_notes" autocomplete="off">
                        <div class="overflow-hidden rounded-lg border border-slate-300 dark:border-slate-600">
                            <div class="flex items-center gap-1 border-b border-slate-200 bg-slate-50 px-2 py-1.5 dark:border-slate-600 dark:bg-slate-800/80">
                                <button type="button" class="amt-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="bold" title="Bold"><span class="font-bold">B</span></button>
                                <button type="button" class="amt-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="italic" title="Italic"><span class="italic">I</span></button>
                                <button type="button" class="amt-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="underline" title="Underline"><span class="underline">U</span></button>
                                <span class="mx-1 h-5 w-px bg-slate-300 dark:bg-slate-600"></span>
                                <button type="button" class="amt-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="insertOrderedList" title="Numbered list">
                                ...
                                <button type="button" class="amt-notes-btn rounded p-2 text-slate-600 transition-colors hover:bg-slate-200 dark:text-slate-400 dark:hover:bg-slate-700" data-cmd="insertUnorderedList" title="Bullets">
                            ...
                            <div id="amt-notes-body" contenteditable="true" data-placeholder="Write notes here"
                ...
                <div class="p-5">
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            ...
                            <label for="amt_upload_plans" class="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50/50 py-4 px-3 text-center transition-colors hover:border-emerald-400 hover:bg-emerald-50/50 dark:border-slate-600 dark:bg-slate-800/50 dark:hover:border-emerald-600 dark:hover:bg-emerald-950/30">
                                ...
                                <input type="file" id="amt_upload_plans" name="upload_plans[]" class="hidden" multiple tabindex="-1">
                            </label>
                            <div id="amt-plans-file-list" class="mt-2 min-h-0 space-y-1 text-xs text-slate-600 dark:text-slate-400"></div>
                            <button type="button" id="amt-plans-clear" class="mt-1 hidden text-xs text-slate-500 hover:text-red-600 dark:text-slate-400 dark:hover:text-red-400">Clear</button>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">Upload Document</label>
                            <label for="amt_upload_document" class="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50/50 py-4 px-3 text-center transition-colors hover:border-emerald-400 hover:bg-emerald-50/50 dark:border-slate-600 dark:bg-slate-800/50 dark:hover:border-emerald-600 dark:hover:bg-emerald-950/30">
                                ...
                                <span class="mt-0.5 text-xs text-slate-500 dark:text-slate-500">or</span>
                                <span class="rounded bg-emerald-500/20 px-2 py-0.5 text-xs font-medium text-emerald-600 dark:text-emerald-400">browse</span>
                                <input type="file" id="amt_upload_document" name="upload_document[]" class="hidden" multiple tabindex="-1">
                            </label>
                            <div id="amt-docs-file-list" class="mt-2 min-h-0 space-y-1 text-xs text-slate-600 dark:text-slate-400"></div>
                            <button type="button" id="amt-docs-clear" class="mt-1 hidden text-xs text-slate-500 hover:text-red-600 dark:text-slate-400 dark:hover:text-red-400">Clear</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="{{ route('amt.list') }}" class="rounded-xl border border-slate-300 px-5 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800">Cancel</a>
            </div>
        </form>
    </div>
@endsection
...
@push('scripts')
    ...
    <script>
        $(function() {
            var notesBody = document.getElementById('amt-notes-body');
            var noteBtns = document.querySelectorAll('.amt-notes-btn');
            ...
            function initBluinqSelect2() {
                $('#amtAddForm select:not(.assignment-user-select)').each(function() {
                    var $el = $(this);
            ...
            renderFileList('amt_upload_plans', 'amt-plans-file-list', 'amt-plans-clear');
            renderFileList('amt_upload_document', 'amt-docs-file-list', 'amt-docs-clear');
            ...
            $btn.on('click', function(e) {
                ...
                var headerRef = ($('#amtJobReferenceContent').text() || '').trim();
                if (headerRef) {
                ...
                $.ajax({
                    ...
                    success: function(resp) {
                        if (resp && resp.status === 'success') {
                            ...
                            formEl.reset();
                            notesBody.innerHTML = '';
                            document.getElementById('amt_upload_plans') && document.getElementById('amt_upload_plans').dispatchEvent(new Event('change'));
                            document.getElementById('amt_upload_document') && document.getElementById('amt_upload_document').dispatchEvent(new Event('change'));
                            initBluinqSelect2();
                            showBluinqAfterSavePrompt(resp.job_id);
                        } else {
            ...
            function showBluinqAfterSavePrompt(jobId) {
                var sendSlackUrl = '{{ url("dashboard/amt/job") }}/' + jobId + '/send-slack';
                var listUrl = '{{ route("amt.list") }}';
                    ...
                    '<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 lbs-after-save-overlay">' +
                        '<div class="w-full max-w-sm rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800 overflow-hidden lbs-after-save-dialog">' +
                            '<div class="p-6 text-center lbs-modal-step lbs-modal-step-question">' +
                                ...
                                '<p class="mt-4 text-sm text-slate-500 dark:text-slate-400">Do you want to create another A&amp;M job?</p>' +
                                '<div class="mt-6 flex gap-3">' +
                                    '<button type="button" data-amt-go-list class="cursor-pointer flex-1 rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-700">Go to A&amp;M list</button>' +
                                    '<button type="button" data-amt-new-job class="cursor-pointer flex-1 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-emerald-500">Create another job</button>' +
                                '</div>' +
                            '</div>' +
                ...
                function proceedAfterSlack(action) {
                    ...
                    var $goListBtn = $overlay.find('[data-amt-go-list]');
                    var $newJobBtn = $overlay.find('[data-amt-new-job]');
                ...
                $overlay.find('[data-amt-new-job]').on('click', function() {
                    proceedAfterSlack('stay');
                });

                $overlay.find('[data-amt-go-list]').on('click', function() {
