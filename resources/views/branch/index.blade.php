...
@section('body_class', 'page-branch-index')

@section('content')
    <div class="priority-page priority-page-enter">
        <div class="priority-header">
            <div class="priority-header-text">
                <h1 class="priority-title">Branch</h1>
                <p class="priority-subtitle">View and manage branch records.</p>
            </div>
            <a href="{{ route('branch.create') }}" class="btn-priority-add">
        ...
        <div class="priority-table-card">
            <div class="priority-table-wrap">
                <table class="priority-table" id="branchTable">
                    <colgroup>
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="priority-th">Branch Name</th>
                    ...
                    <tbody>
                        @forelse($branches as $branch)
                            <tr>
                                <td class="priority-td">{{ $branch->id }}</td>
                                <td class="priority-td">{{ $branch->branch_name }}</td>
                                <td class="priority-td">{{ $branch->created_at?->format('M j, Y g:i A') ?? '—' }}</td>
                                <td class="priority-td">{{ $branch->updated_at?->format('M j, Y g:i A') ?? '—' }}</td>
                                <td class="priority-td priority-td-action">
                                    <div class="priority-action-btns">
                                        <a href="{{ route('branch.edit', $branch) }}" class="priority-action-icon priority-action-edit" title="Edit" aria-label="Edit">
                                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                        </a>
                                        <form action="{{ route('branch.destroy', $branch) }}" method="POST" class="priority-delete-form" data-delete-form autocomplete="off">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="priority-td priority-td-empty">No branch records yet. <a href="{{ route('branch.create') }}">Add one</a>.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="modal-backdrop" id="deleteBranchModal" role="dialog" aria-labelledby="deleteBranchModalTitle" aria-modal="true">
            <div class="modal-box">
                <div class="modal-header">
                    ...
                    <h2 class="modal-title" id="deleteBranchModalTitle">Delete Branch</h2>
                </div>
                <div class="modal-body">
                    <div class="delete-modal-confirm" id="deleteBranchModalConfirmBlock">
                        <p>Are you sure you want to delete this branch? This action cannot be undone.</p>
                    </div>
                    <div class="delete-modal-countdown" id="deleteBranchModalCountdown" hidden>
                        <p class="delete-countdown-text">Deleting in</p>
                        <div class="delete-countdown-number" id="deleteBranchCountdownNumber">3</div>
                        <p class="delete-countdown-cancel-hint">Click Cancel to abort</p>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-cancel" id="deleteBranchModalCancel">Cancel</button>
                    <button type="button" class="btn btn-confirm btn-danger" id="deleteBranchModalConfirm"><span class="btn-text">Delete</span></button>
                </div>
            </div>
        </div>
    </div>
...
@push('styles')
    <style>
        ...
        body.page-branch-index .content { padding-bottom: 0; }
...
@push('scripts')
    <script>
        (function() {
            var modal = document.getElementById('deleteBranchModal');
            var cancelBtn = document.getElementById('deleteBranchModalCancel');
            var confirmBtn = document.getElementById('deleteBranchModalConfirm');
            var confirmBlock = document.getElementById('deleteBranchModalConfirmBlock');
            var countdownBlock = document.getElementById('deleteBranchModalCountdown');
            var countdownNumber = document.getElementById('deleteBranchCountdownNumber');
