...
@section('body_class', 'page-lbs-list page-leading-energy-list page-leading-energy-review')

@section('content')
    @php
        $__assign = \App\Models\User::assignmentInitialsViewData('leading_energy');
...
<div class="block max-w-full pb-0">
        <div class="mb-7 flex flex-wrap items-start justify-between gap-4">
            <div class="min-w-0">
                <h1 class="m-0 mb-1.5 text-[1.625rem] font-bold tracking-tight text-slate-900 dark:text-white">Leading Energy For Review</h1>
                <p class="m-0 text-[0.9375rem] leading-snug text-slate-600 dark:text-slate-400">View Leading Energy jobs that are for review.</p>
            </div>
            <div class="shrink-0">
                ...
                <div class="relative flex min-w-[260px] items-center">
                    ...
                    <input type="search" id="lbsSearch" class="w-full rounded-lg border border-slate-300 bg-white py-2 pl-9 pr-3.5 text-sm text-slate-900 placeholder-slate-500 transition-colors focus:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-600/25 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:placeholder-slate-500 dark:focus:border-blue-700 dark:focus:ring-blue-700/25" placeholder="Search by client, reference, job type..." autocomplete="off" aria-label="Search Leading Energy review jobs">
                </div>
            </div>
        </div>
                ...
                <table class="lbs-table w-full min-w-[1320px] table-fixed border-collapse text-sm" id="lbsTable">
                    ...
                    <tbody>
                        ...
                        @foreach($rows as $row)
                            ...
                            @endphp
                            <tr class="lbs-data-row border-b border-slate-200 align-middle text-slate-800 dark:border-slate-700 dark:text-slate-200" data-job-units="{{ (int) ($row->units ?? 0) }}" data-update-url="{{ route('leading_energy.update', ['id' => $row->id], false) }}">
                                <td class="px-4 py-3 text-center"><a href="{{ route('leading_energy.view', ['id' => $row->id]) }}" class="lbs-action-icon inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-green-500/15 hover:text-green-400" title="View"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></a></td>
