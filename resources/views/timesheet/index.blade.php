    ...
    <div class="w-full max-w-full space-y-4">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Team leave overview — approved leave only</p>
            </div>
        </div>
                        ...
                        @forelse($team as $member)
                            <tr class="border-b border-slate-100 last:border-b-0 dark:border-slate-700/70">
                                ...
                                @foreach($days as $day)
                                    ...
                                    @endphp
                                    <td class="px-0.5 py-1.5 text-center align-middle {{ $day['is_weekend'] ? 'bg-slate-50 dark:bg-slate-900/40' : '' }}" title="{{ $onLeave ? ucfirst($leaveType).' · '.$day['date'] : $day['date'] }}">
                                        @if($onLeave)
                                            <span class="mx-auto inline-flex h-7 w-7 items-center justify-center rounded-md bg-amber-400/90 text-[10px] font-bold uppercase text-amber-950">
                                                {{ mb_strtoupper(mb_substr((string) $leaveType, 0, 1)) }}
                                            </span>
                                        @elseif($day['is_today'])
                                            <span class="mx-auto inline-flex h-7 w-7 items-center justify-center rounded-full bg-sky-600 text-[11px] font-semibold text-white">{{ $day['day'] }}</span>
                                        @else
                                            <span class="mx-auto inline-flex h-7 w-7 items-center justify-center text-[11px] {{ $day['in_month'] ? 'text-slate-300 dark:text-slate-600' : 'text-slate-200 dark:text-slate-700' }}">{{ $day['day'] }}</span>
                                        @endif
                                    </td>
                                @endforeach
                            </tr>
        ...
        <p class="text-xs text-slate-400 dark:text-slate-500">
            Green number under each name is remaining leave credits. Amber cells mark approved leave days.
        </p>
    </div>
@endsection
